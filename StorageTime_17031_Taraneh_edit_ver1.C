...
void StorageTime_17031_Taraneh_edit_ver1(){
  gStyle->SetOptFit(1111);
  // READ THE TREE
  
  TFile *fin = new TFile("outputTree_StorageTime_17031.root","READ");
  if ( fin->IsOpen() ){
  ...
  double HistIntegralArray[counts];
  ...
  TH2* StorageTime = new TH2F("StorageTime", "Counts vs Delay Time", 1000, 0., 190. , 1000,  0., 200000);
  ...
  //TH2* UCN = new TH2F("UCN", "UCN vs run#", 1000, 780, 790 , 1000,  0., 50000);
  
  // HISTOGRAM TO SHOW THE AVERAGE ISOPURE TEMPERATURE VS THE RUN NUMBER
  //TH2* runTemp = new TH2F("UCN", "Temperature vs run#", 1000, 780, 790,1000, 0.83, 1.1);
  
  // HISTOGRAM TO SHOW THE TOTAL IRRADIATION TIME VS THE AVERAGE ISOPURE TEMPERATURE
  TH2* IrradTemp = new TH2F("UCN", "Irradiation Time vs Temperature", 1000, 0, 200 , 1000, 0.83, 0.95);
  
  // HISTOGRAM TO SHOW THE AVERAGE ISOPURE TEMPERATURE VS BEAM CURRENT
  TH2* TempBeam = new TH2F("UCN", "Temperature vs Beam Current", 1000, 0.83, 0.95 , 1000, 98, 101);
  
  // HISTOGRAM TO SHOW THE THE TOTAL UCN (ONLY) COUNTS VS AVERAGE BEAM CURRENT
  TH2* UCNBeam = new TH2F("UCN", "UCN Counts vs Beam Current" , 1000, 98, 101 , 1000 , 0 , 200000 );
  
  // HISTOGRAM TO SHOW THE TOTAL UCN (ONLY) COUNT VS IRRADIATION TIME
  TH2* UCNIrrad = new TH2F("UCN", "UCN Counts vs Irradiation Time" , 1000, 0, 200 , 1000 , 0 , 200000 );


  TF1 *Func1 = new TF1("Func1", "[0]*(1-exp(-x/[1]))*exp(-x/[2])", 0, 180);
  double integralArray[100];
  double integralErrArray[100];
  ...
  for (ULong64_t j = 0 ; j < events ; j++){
    ...
    UCNIrrad -> Fill (irradTime, UCNIntegral);
    delayTimeArray[j] = delayTime;
    UCNIntegralArray[j] = UCNIntegral;
    UCNIntegralErrArray[j] = UCNIntegralErr;
    HistIntegralArray[j] = HistIntegral;
    cout <<  HistIntegralArray[j] << " " << UCNIntegralArray[j]<< endl;
  }
  ...
  TF1 *fit = new TF1 ("fit" , "[0]*exp(-x/[1])", 0, 180);
  fit -> SetParameters(140000,40);
  ...
  text -> SetTextAngle(0);
  text -> DrawText(0.4, 0.8, "Exp #: TCN17031");
  ...
  // c1 -> cd(2);
  StorageTemp-> GetXaxis()-> SetTitle("Average Isopure Temperature" );
  ...
  TGraphErrors *gr = new TGraphErrors(10, delayTimeArray, UCNIntegralArray, 0 , UCNIntegralErrArray);
  c6 -> SetLogy(1);
  gr -> GetYaxis()-> SetRangeUser(1,200000);
  ...
  gr -> Fit(fit);
   ...
   TGraphErrors *gr2 = new TGraphErrors(10, delayTimeArray, HistIntegralArray,0,0);
  ...
  UCNIrrad -> GetYaxis()-> SetTitle("UCN Count");
  UCNIrrad -> GetXaxis()-> SetTitle("Irradiaiton Time (s)");
  UCNIrrad -> SetMarkerStyle(7);
  UCNIrrad -> Draw();

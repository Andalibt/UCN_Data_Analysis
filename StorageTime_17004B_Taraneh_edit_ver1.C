...
void StorageTime_17004B_Taraneh_edit_ver1(){
  gStyle->SetOptFit(1111);
  // READ THE TREE
  
  TFile *fin = new TFile("outputTree_StorageTime_17004B.root","READ");
  if ( fin->IsOpen() ){
  ...
  TH2* UCN = new TH2F("UCN", "UCN vs run#", 1000, 570, 585 , 1000,  0., 200000);
  // HISTOGRAM TO SHOW THE AVERAGE ISOPURE TEMPERATURE VS THE RUN NUMBER
  TH2* runTemp = new TH2F("UCN", "Temperature vs run#", 1000, 570, 585,1000, 0.83, 0.98);
  ...
  text -> SetTextAngle(0);
  text -> DrawText(0.4, 0.8, "Exp #: TCN17004B");

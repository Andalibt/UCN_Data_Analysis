...
void plot_ucn_per_cycle_Taraneh_edit_StorageTime_TCN17003(){  
  // Get the file
  char *filename[200];
  ...
  TFile hfile ("outputTree_StorageTime_17003.root", "RECREATE");
 ...
 TH2F *psd_vs_ql_withcuts[16];
  ...
  Int_t StorageTimeFiles[173] = {541, 542, 543, 544, 545, 546};
  ...
  for ( midasrun =524; midasrun <838; midasrun++){
	...
	for(ULong64_t j=0; j < beamlineEvent ;j++) {
	  ...
	  if (timestamp_beamline > irradiationStartTime[i] && timestamp_beamline <= cycleStartTimes[i]) {
	    //cout << "CYCLE " << i << " " << B1V_KSM_PREDCUR << endl;
	...
	//Li6_Fit_Func-> Draw("");
	//cout << numberEventsPerCycleValveOpen[i] << " " << Li6_integral[i] << " " << Integral[i] << endl;
	hfile.cd();
      }

...
void StorageTime_17009_Taraneh_edit_ver1(){
  gStyle->SetOptFit(1111);
  // READ THE TREE
  
  TFile *fin = new TFile("outputTree_StorageTime_17009.root","READ");
  if ( fin->IsOpen() ){
  ...
  TH2* UCN = new TH2F("UCN", "UCN vs run#", 1000, 660, 700 , 1000,  0., 600000);
  
  // HISTOGRAM TO SHOW THE AVERAGE ISOPURE TEMPERATURE VS THE RUN NUMBER
  TH2* runTemp = new TH2F("UCN", "Temperature vs run#", 1000, 660, 700,1000, 0.83, 1.1);
  
  // HISTOGRAM TO SHOW THE TOTAL IRRADIATION TIME VS THE AVERAGE ISOPURE TEMPERATURE
  TH2* IrradTemp = new TH2F("UCN", "Irradiation Time vs Temperature", 1000, 30, 200 , 1000, 0.83, 1.3);
  
  // HISTOGRAM TO SHOW THE AVERAGE ISOPURE TEMPERATURE VS BEAM CURRENT
  TH2* TempBeam = new TH2F("UCN", "Temperature vs Beam Current", 1000, 0.83, 1.3 , 1000, 99, 140);
  
  // HISTOGRAM TO SHOW THE THE TOTAL UCN (ONLY) COUNTS VS AVERAGE BEAM CURRENT
  TH2* UCNBeam = new TH2F("UCN", "UCN Counts vs Beam Current" , 1000, 99, 140 , 1000 , 0 , 600000 );
  ...
  text -> SetTextAngle(0);
  text -> DrawText(0.4, 0.8, "Exp #: TCN17009");
  ...
  TGraphErrors *gr = new TGraphErrors(26, delayTimeArray, UCNIntegralArray, 0 , UCNIntegralErrArray);

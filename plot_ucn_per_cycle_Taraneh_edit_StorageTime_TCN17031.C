...
// TCN17031 WHICH IS A STORAGE LIFETIME MEASRUEMENT.
//*******************************************************
...
void plot_ucn_per_cycle_Taraneh_edit_StorageTime_TCN17031(){  
  // Get the file
  char *filename[200];
  ...
  TFile hfile ("outputTree_StorageTime_17031.root", "RECREATE");
 ...
 TH2F *psd_vs_ql_withcuts[16];
  ...
  // Int_t StorageTimeFiles[173] = {834};
  ...
  for ( midasrun =834; midasrun <835; midasrun++){
    total_counter ++;
    runNumber = midasrun;
    runs++;
    cout << midasrun << endl;
	...
	for ( int i = 0; i < cycleStartTimes.size(); i++){
	  ...
	  Bin_high[i] = UCN_rate_li6 ->  GetXaxis() -> FindBin(cyclevalveclose[i]);
	    ...
	    //cout << Bin_low[i] << " " << Bin_high[i] << endl;

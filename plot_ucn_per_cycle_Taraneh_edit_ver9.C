...
Double_t Li6FitFunc (Double_t *x, Double_t *par){
  ...
  Li6_Fit = par[3] + (x[0] > par[0] && x[0] < par[2]) * par[1]*par[4] + (x[0] > par[2] + par[9]+ par[1s1]) * par[6]*(1 - exp(-(x[0] - par[2] - par[11]- par[9])/par[7]))*exp(-(x[0] - par[2] - par[11] - par[9])/par[8]);
...
void plot_ucn_per_cycle_Taraneh_edit_ver9(){  
  // Get the file
  char *filename[200];
  ...
  TFile hfile ("outputTree_ver9.root", "RECREATE");
 ...
 TH2F *psd_vs_ql_withcuts[16];
  ...
  for(int i = 0; i < StartofCycle.size(); i++) RiseTime.push_back(0.);
  std::vector <double> FallTime;
  for(int i = 0; i < StartofCycle.size(); i++) FallTime.push_back(0.);
  std::vector <double> DELAY;
  for(int i = 0; i < StartofCycle.size(); i++) DELAY.push_back(0.);
  ...
  for ( midasrun =524; midasrun <838; midasrun++){
    ...
    // printf("%s\n",filename); 
    if (midasrun == 526)  // THE UCN RATE HAS PEAKS AND SHOULDERS.
      continue;
    if (midasrun == 530)  // LOOKS ALMOST FLAT! FIT DOES NOT WORK
      continue;
    if (midasrun == 536)  // IT SAYS IT HAS 6 CYCLES BUT IT ACTUALLY IS 4.
      continue;
    if (midasrun == 538)  // UCN RATE HAS DOUBLE SHOULDER, IGNORED FOR FITTING ISSUES
      continue;
    if (midasrun == 539)
      continue;
    if (midasrun == 540)  // THE RATE HAS DOULE PEAK.
      continue;
    if (midasrun == 541) // IT SAYS IT HAS 3 CYCLES BUT I ONLY SEE 2 AND THE THIRD CYCLE'S FIT FAILS
      continue;
    if (midasrun == 542) //AFTER ADDING FIT FAILS
      continue;
    if (midasrun == 543) //AFTER ADDING FIT FAILS
      continue;
    if (midasrun == 544) //AFTER ADDING FIT FAILS
      continue;
    if (midasrun == 545) //AFTER ADDING FIT FAILS
      continue;
    if (midasrun == 546) //AFTER ADDING FIT FAILS
      continue;
    if (midasrun == 547) //IT SAYS THREE CYCLES BUT THERE ARE ONLY TWO AND THE THIRD FIT FAILS.
      continue;
    if (midasrun == 548) // THIS RUN HAS 6 CYCLES AND THE LAST CYCLE IS STRANGE BECAUSE THE IRRADIAITION
      continue;          // START TIME IS WHEN THE RISE HAPPENS IN THE UCN RATE. SO THE VALVE MUST BE LEFT OPEN. 
    if (midasrun == 549)
      continue;
    if (midasrun == 552) // FIT FOR THE 2ND AND 4TH CYCLES FAIL FOR UNKNOWN REASONS.
      continue;
    if (midasrun == 560)
      continue;
    if (midasrun == 566)// THE FIRST CYCLE FAILS FOR UNKNOWN REASON.
      continue;
    if (midasrun == 568)// THE LAST CYCLE FAILS FOR UNKNOWN REASON.
      continue;
    if (midasrun == 571)// THE LAST CYCLE FAILS FOR UNKNOWN REASON.
      continue;
    if (midasrun == 572)// THE SECOND CYCLE FAILS FOR UNKNOWN REASON.
      continue;
    if (midasrun == 574) //AFTER ADDING FIT FAILS
      continue;
    if (midasrun == 575) //AFTER ADDING FIT FAILS
      continue;
    if (midasrun == 576) //AFTER ADDING FIT FAILS
      continue;
    if (midasrun == 577) //AFTER ADDING FIT FAILS
      continue;
    if (midasrun == 578) //AFTER ADDING FIT FAILS
      continue;
    if (midasrun == 579) //AFTER ADDING FIT FAILS
      continue;
    if (midasrun == 580) //AFTER ADDING FIT FAILS
      continue;
    if (midasrun == 581) //AFTER ADDING FIT FAILS
      continue;
    if (midasrun == 582) //AFTER ADDING FIT FAILS
      continue;
    if (midasrun == 585) // THE UCN RATE HISTOGRAM LOOKS ODD, IT IS A DIFFERENT MEASUREMENT?? 
      continue;
    if (midasrun == 587) // THE UCN RATE HISTOGRAM LOOKS ODD, IT IS A DIFFERENT MEASUREMENT?? 
      continue;
    if (midasrun == 588) // THE UCN RATE HISTOGRAM LOOKS ODD, IT IS A DIFFERENT MEASUREMENT?? 
      continue;
    if (midasrun == 589) // THE UCN RATE HISTOGRAM LOOKS ODD, IT IS A DIFFERENT MEASUREMENT?? 
      continue;
    if (midasrun == 590) // THE UCN RATE HISTOGRAM LOOKS ODD, IT IS A DIFFERENT MEASUREMENT?? 
      continue;
    if (midasrun == 591) // THE UCN RATE HISTOGRAM LOOKS ODD, IT IS A DIFFERENT MEASUREMENT?? 
      continue;
    if (midasrun == 592) // THE UCN RATE HISTOGRAM LOOKS ODD, IT IS A DIFFERENT MEASUREMENT?? 
      continue;
    if (midasrun == 593) // THE UCN RATE HISTOGRAM LOOKS ODD, IT IS A DIFFERENT MEASUREMENT?? 
      continue;
    if (midasrun == 594) // THE UCN RATE HISTOGRAM LOOKS ODD, IT IS A DIFFERENT MEASUREMENT?? 
      continue;
    if (midasrun == 595) //AFTER ADDING FIT FAILS
      continue;
    if (midasrun == 596) // IT SAYS THERE ARE 4 CYCLES BUT THE UCN RATE SHOWS ONLY ONE!
      continue;
    if (midasrun == 600) //AFTER ADDING FIT FAILS
      continue;
    if (midasrun == 601) //AFTER ADDING FIT FAILS
      continue;
    if (midasrun == 602) //AFTER ADDING FIT FAILS
      continue;
    if (midasrun == 614) // ONLY FITS THE MIDDLE CYCLE FOR UNKNOWN REASONS.
      continue;
    ...
    if (midasrun == 618) // I HAVE NO IDEA WHAT THIS RUN IS
      continue;
    if (midasrun == 623) // I HAVE NO IDEA WHAT IS HAPPENING DURING THIS RUN!
      continue;
    if (midasrun == 624) // THIS RUN I THINK IS JUST CRAP WITH 0 CYCLES.
      continue;
    if (midasrun == 628) //AFTER ADDING FIT FAILS
      continue;
    if (midasrun == 631) // I CANNOT MAKE THE FIT WORK FOR SOME REASON.
      continue;
    if (midasrun == 632) // I AM NOT SURE WHAT IS GOING ON ON THIS RUN.
      continue;
    if (midasrun == 633) //AFTER ADDING FIT FAILS
      continue;
    ...
    if (midasrun == 641) // AFTER ADDING FIT FAILS
      continue;
    if (midasrun == 642) // THERE ARE 0 CYCLES IN THIS FILE. IT IS CRAP.
      continue;
    if (midasrun == 643) // THERE ARE 0 CYCLES IN THIS FILE. IT IS CRAP.
      continue;
    if (midasrun == 651) // I AM NOT SURE WHAT IS HAPPENING DURING THIS RUN! IT LOOKS STRANGE.
      continue;
    if (midasrun == 652) // THE UCN RATE IN THIS RUN LOOKS GOOD. HOWEVER, IT SAYS 0 CYCLES AND FIT 
      continue;          // DOES NOT EVEN START.
    if (midasrun == 653) // THE UCN RATE IN THIS RUN LOOKS GOOD. HOWEVER, IT SAYS 0 CYCLES AND FIT 
      continue;          // DOES NOT EVEN START.
    if (midasrun == 654) // THE UCN RATE LOOKS STRANGE AND IT HAS 0 CYCLES.
      continue;
    if (midasrun == 655) // THE UCN RATE LOOKS STRANGE AND IT HAS 0 CYCLES.
      continue;
    if (midasrun == 656) // THE UCN RATE LOOKS STRANGE AND IT HAS 0 CYCLES.
      continue;
    if (midasrun == 657) // THE UCN RATE LOOKS STRANGE AND IT HAS 0 CYCLES.
      continue;
    if (midasrun == 658) // THE UCN RATE LOOKS STRANGE AND IT HAS 0 CYCLES.
      continue;
    if (midasrun == 659) // THE UCN RATE LOOKS STRANGE AND IT HAS 0 CYCLES.
      continue;
    if (midasrun == 668) // THE UCN RATE LOOKS STRANGE.
      continue;
    if (midasrun == 672) // THE UCN RATE LOOKS STRANGE.
      continue;
    if (midasrun == 673) // THE UCN RATE LOOKS STRANGE.
      continue;
    if (midasrun == 674)
      continue;
    if (midasrun == 677) // THE FIT FAILS FOR UNKNOWN REASON.
      continue;
    if (midasrun == 678) // THE FIT FAILS FOR UNKNOWN REASON.
      continue;
    if (midasrun == 690) // THE UCN RATE LOOKS STRANGE AND IT HAS 0 CYCLES.
      continue;
    if (midasrun == 691)
      continue;
    if (midasrun == 694)
      continue;
    if (midasrun == 698) // THE FIT FAILS FOR UNKNOWN REASON.
      continue;
    if (midasrun == 700) // IT HAS 0 CYCLES IN IT.
      continue;
    if (midasrun == 703) // THE FIT FAILS FOR UNKNOWN REASON.
      continue;
    if (midasrun == 704) // THE UCN RATE LOOKS STRANGE.
      continue;
    if (midasrun == 712) // THE FIT FAILS FOR UNKNOWN REASON.
      continue;
    if (midasrun == 713) // THE FIT FAILS FOR UNKNOWN REASON.
      continue;
    if (midasrun == 719) // THE FIT FAILS FOR UNKNOWN REASON.
      continue;
    if (midasrun == 721) // THE UCN RATE LOOKS STRANGE AND THE FIT FAILS.
      continue;
    if (midasrun == 722) // THE FIT FAILS FOR UNKNOWN REASON.
      continue;
    if (midasrun == 729) // THE UCN RATE LOOKS STRANGE AND THE FIT FAILS.
      continue;
    if (midasrun == 741) // THE FIT FAILS FOR UNKNOWN REASON.
      continue;
    if (midasrun == 742) // THE FIT FAILS FOR UNKNOWN REASON.
      continue;
    if (midasrun == 749) // AFTER ADDING FIT FAILS
      continue;
    if (midasrun == 751) // THE UCN RATE LOOKS SO STRANGE.
      continue;
    if (midasrun == 761) // AFTER ADDING FIT FAILS
      continue;
    if (midasrun == 762) // THE FIT FAILS FOR UNKNOWN REASON.
      continue;
    if (midasrun == 763) // THE FIT FAILS FOR UNKNOWN REASON.
      continue;
    if (midasrun == 764) // THE FIT FAILS FOR UNKNOWN REASON.
      continue;
    if (midasrun == 766) // THE FILE HAS 0 CYCLES.
      continue;
    if (midasrun == 767) // THE FILE HAS 0 CYCLES.
      continue;    
    if (midasrun == 768) // THE FILE HAS 0 CYCLES.
      continue;
    if (midasrun == 769) // THE FILE HAS 0 CYCLES.
      continue;
    if (midasrun == 770) // THE FILE HAS 0 CYCLES.
      continue;
    if (midasrun == 771) // THE FILE HAS 0 CYCLES.
      continue;
    if (midasrun == 772) // THE FILE HAS 0 CYCLES.
      continue;
    if (midasrun == 773) // THE FILE HAS 0 CYCLES.
      continue;
    if (midasrun == 774) // AFTER ADDING FIT FAILS
      continue;
    if (midasrun == 775)
      continue;
    if (midasrun == 776) // THE FIT FAILS FOR UNKNOWN REASON.
      continue;
    if (midasrun == 777) // SOME FITS FAIL AND SOME DO NOT AND I DO NOT KNOW WHY.
      continue;
    if (midasrun == 778) // THE FILE HAS 0 CYCLES.
      continue;
    if (midasrun == 779) // THE FILE HAS 0 CYCLES.
      continue;
    if (midasrun == 780) // THE UCN RATE HISTOGRAM LOOKS VERY STRANGE.
      continue;
    if (midasrun == 781) // IT SAYS IT HAS 9 CYCLES BUT I ONLY SEE 8 AND ALSO FIT FAILS FOR THE LAST CYCLE
      continue;
    if (midasrun == 782) // IT SAYS IT HAS 10 CYCLES BUT I ONLY SEE 9 AND THE FIT FAILS.
      continue;
    if (midasrun == 783) // THE FILE HAS 0 CYCLES.
      continue;
    if (midasrun == 784) // IT SAYS IT HAS 8 CYCLES BUT I ONLY SEE 7 AND THE FIT FAILS.
      continue;
    if (midasrun == 785) // I DON'T KNOW WHY THE FIT FAILS. THE RATE HISTOGRAM LOOKS OK.
      continue;
    if (midasrun == 786) // IT SAYS IT HAS 8 CYCLES BUT I ONLY SEE 7 AND THE FIT FAILS.
      continue;
    if (midasrun == 787) 
      continue;
    if (midasrun == 788) // THIS RUN HAS 0 CYCLES. 
      continue;
    if (midasrun == 789) // IT SAYS IT HAS 21 CYCLES BUT I ONLY SEE 9!
      continue;
    if (midasrun == 790) // IT SAYS IT HAS 8 CYCLES BUT I ONLY SEE 6.
      continue;
    if (midasrun == 791) // IT SAYS IT HAS 12 CYCLES BUT I ONLY SEE 9.
      continue;
    ...
    if (midasrun == 794)  // THE RATE HISTOGRAM IS STRANGE, VERY LOW INTENSITY AND THE FIT FAILS.  
      continue;
    if (midasrun == 795)  // IT HAS 0 CYCLES. 
      continue;
    if (midasrun == 796)  // AFTER ADDING FIT FAILS
      continue;
    if (midasrun == 797)
      continue;
    if (midasrun == 798)  // IT HAS 0 CYCLES. 
      continue;
    if (midasrun == 800)  // IT HAS 40 CYCLES. MOST OF THEM FIT RIGHT BUT A FEW FAIL.
      continue;
    if (midasrun == 801)  // IT HAS 40 CYCLES. MOST OF THEM FIT RIGHT BUT A FEW FAIL.
      continue;
    if (midasrun == 802)  // IT HAS 40 CYCLES. MOST OF THEM FIT RIGHT BUT A FEW FAIL.
      continue;
    if (midasrun == 803)  // IT SAYS IT HAS 9 CYCLES BUT ONLY 7 ARE VISIBLE IN THE RATE HISTOGRAM.
      continue;           // AND AS A RESULT SOME FIT CYCLES FAIL.
    if (midasrun == 804)  // IT HAS 0 CYCLES.
      continue;
    if (midasrun == 805)  // IT HAS 0 CYCLES.
      continue;
    if (midasrun == 806)  // IT HAS 0 CYCLES.
      continue;
    if (midasrun == 807)  // IT HAS 0 CYCLES.
      continue;
    if (midasrun == 808)  // IT HAS 0 CYCLES.
      continue;
    if (midasrun == 809)  // THE RATE HISTOGRAM LOOKS STRANGE.
      continue;
    if (midasrun == 810)
      continue;
    if (midasrun == 811)  // IT HAS 32 CYCLES. MOST FIT EXCEPT A FEW AND I AM NOT SURE WHY.
      continue;
    if (midasrun == 812)  // IT HAS 15 CYCLES AND ONLY 6 OF THEM FIT.
      continue;
    if (midasrun == 813)  // IT HAS 10 CYCLES BUT 2 OF THEM DO NOT REALLY EXIST.
      continue;
    if (midasrun == 814)  // IT HAS 50 CYCLES. THE RATE HISTOGRAM LOOKS STRANGE WITH DOUBLE PEAKS,
      continue;           // AND THE FIT FAILS AS A RESULT.
    if (midasrun == 815)  // IT HAS 0 CYCLES.
      continue;
    if (midasrun == 816)  // IT HAS 0 CYCLES.
      continue;
    if (midasrun == 817)  // IT HAS 0 CYCLES.
      continue;
    if (midasrun == 818)  // IT HAS 0 CYCLES.
      continue;
    if (midasrun == 819)  // IT SAYS IT HAS 19 CYCLES BUT I CAN SEE 16 AT MOST.
      continue;
    if (midasrun == 821)  // IT SAYS 10 CYCLES BUT ONLY 8 ARE VISIBLE.
      continue;
    if (midasrun == 822)  // IT HAS 18 CYCLES AND FOR A FEW OF THEM THE FIT FAILS.
      continue;
    if (midasrun == 823)  // IT HAS 29 CYCLES AND FOR A FEW OF THEM THE FIT FAILS.
      continue;
    if (midasrun == 824)  // IT HAS 33 CYCLES AND FOR SOME OF THEM THE FIT FAILS.
      continue;
    if (midasrun == 825)  // IT HAS 0 CYCLES.
      continue;
    if (midasrun == 826)  // IT HAS 0 CYCLES.
      continue;
    if (midasrun == 827)  // IT SAYS IT HAS 9 CYCLES BUT I CAN ONLY SEE 8.
      continue;
    if (midasrun == 829)  // IT HAS 45 CYCLES AND MOST FITS FAIL.
      continue;
    if (midasrun == 830)  // THE RATE HISTOGRAM LOOKS STRANGE.
      continue;
    if (midasrun == 831)
      continue;
    if (midasrun == 831)  // IT SAYS IT HAS 10 CYCLES BUT I CAN ONLY SEE 8.
      continue;
    if (midasrun == 832)  // IT HAS 0 CYCLES.
      continue;
    if (midasrun == 833)  // IT HAS 0 CYCLES.
      continue;
    if (midasrun == 834)  // IT SAYS IT HAS 10 CYCLES BUT I CAN ONLY SEE 8.
      continue;    
    if (midasrun == 835)  // IT HAS 45 CYCLES AND SOME FITS FAIL.
      continue;   
    if (midasrun == 836)  // IT HAS 22 CYCLES AND SOME FITS FAIL.
      continue;
    if (midasrun == 837)  // IT HAS 13 CYCLES AND ONE FIT FAILS.
      continue;      
	...
	if(cycleStartTimehe3.size() > 0){
      ...
      //**********************************************************************
      Double_t baseline[max];
      Double_t baseline_during_irrad[max];
      Double_t amp[max];
      Double_t tauRise[max];
      Double_t tauFall[max];
      Double_t delay[max];
	...
	Li6_Fit_Func -> SetParameter(6, (int)Li6Max[i]*BinWidth);
	Li6_Fit_Func -> SetParLimits(6, Li6Max[i]+2000, Li6Max[i]*BinWidth+2000);
	Li6_Fit_Func -> SetParameter(7, 2);
	Li6_Fit_Func -> SetParLimits(7, 0, 10);
	Li6_Fit_Func -> SetParameter(8, 20);
	Li6_Fit_Func -> SetParLimits(8, 0, 40);
	Li6_Fit_Func -> SetParameter(9, 0.1);
	...
	UCN_rate_li6->Fit(Li6_Fit_Func,"R+M");
	...
	BaselineDuringIrrad.push_back(baseline_during_irrad[i]*BinWidth*prdcur_ave[i]);
	Baseline.push_back(baseline[i]*BinWidth);
	Amplitude.push_back(amp[i]);
	RiseTime.push_back(tauRise[i]);
	FallTime.push_back(tauFall[i]);
	DELAY.push_back(delay[i]);
      }

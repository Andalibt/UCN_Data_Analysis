...
void StorageTime_17002_Taraneh_edit_ver1(){
  gStyle->SetOptFit(1111);
  // READ THE TREE
  
  TFile *fin = new TFile("outputTree_StorageTime_17002.root","READ");
  if ( fin->IsOpen() ){
  ...
  TH2* StorageTemp = new TH2F("StorageTemp", "Counts vs Temperture", 1000, 0.83, 0.93 , 1000,  0., 200000);
  // HISTOGRA TO SHOW THE TOTAL UCN (ONLY) COUNTS VS THE RUN NUMBER
  TH2* UCN = new TH2F("UCN", "UCN vs run#", 1000, 530, 545 , 1000,  0., 200000);
  // HISTOGRAM TO SHOW THE AVERAGE ISOPURE TEMPERATURE VS THE RUN NUMBER
  TH2* runTemp = new TH2F("UCN", "Temperature vs run#", 1000, 530, 545,1000, 0.83, 0.93);
  // HISTOGRAM TO SHOW THE TOTAL IRRADIATION TIME VS THE AVERAGE ISOPURE TEMPERATURE
  TH2* IrradTemp = new TH2F("UCN", "Irradiation Time vs Temperature", 1000, 30, 200 , 1000, 0.83, 0.93);
  // HISTOGRAM TO SHOW THE AVERAGE ISOPURE TEMPERATURE VS BEAM CURRENT
  TH2* TempBeam = new TH2F("UCN", "Temperature vs Beam Current", 1000, 0.83, 0.93 , 1000, 80, 109);
  ...
  text -> SetTextAngle(0);
  text -> DrawText(0.4, 0.8, "Exp #: TCN17002");
  ...
  gr -> SetMarkerStyle(7);
  gr -> Fit(fit , "0");
  TGraphErrors *gr2 = new TGraphErrors(26, delayTimeArray, HistIntegralArray, 0 , 0);

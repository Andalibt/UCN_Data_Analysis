...
// files.
// In this version, you get the run number as an input.
...
void plot_ucn_per_cycle_Taraneh_edit_ver15(int midasrun){
  ...
  TString foutname = Form ("./2ndpass/outputTree_%d.root", midasrun);
  TFile hfile (foutname, "RECREATE");
  ...
  double He3CountsValveOpen;
  double He3CountsCycleStart;
 ...
 TH2F *psd_vs_ql_withcuts[16];
  ...
  TH2* UCN_per_cycle_Li6_All = new TH2F("UCN_per_cycle_Li6_Whole_Sequence", "UCN per cycle", 100, minXrange, maxXrange , 100, 0.0, 10000); // only for the last run
  ...
  outputTree -> Branch ("He3CountsCycleStart" , &He3CountsCycleStart);
  outputTree -> Branch ("He3CountsValveOpen" , &He3CountsValveOpen);
  ...
  //total_counter ++;
    ...
    if ( fin->IsOpen() == false ) {
      ...
      double numberEventsPerCycle[max];
      double numberEventsPerCycleValveOpen[max];
      ULong64_t eventTot;
      //numcycle = 0;
	...
	UCN_rate_He3 -> SetBins(NBins, minXrange , maxXrange);
        ...
        double numberEventsPerCyclehe3[max];
	double numberEventsPerCyclehe3valveOpen[max];
	ULong64_t runtransitionhe3Event;
	ULong64_t He3Event;
	double cycleStart_he3;
	std::vector <double> cycleStartTimes_He3;
	double cycleStartTime_He3[max];
	double cyclevalveopenhe3;
	double cyclevalveclosehe3;
	double cyclevalveopen_He3[max];
	double cyclevalveclose_He3[max];
	  
	runtransitionhe3 -> SetBranchAddress("cycleStartTime", &cycleStart_he3);
	runtransitionhe3 -> SetBranchAddress ("cycleValveOpenTime" ,&cyclevalveopenhe3);
	runtransitionhe3 -> SetBranchAddress ("cycleValveCloseTime" ,&cyclevalveclosehe3);
	...
	for(ULong64_t j=0; j < runtransitionhe3Event ; j++) {
	  ...
	  cycleStartTimes_He3.push_back(cycleStart_he3);
	  cycleStartTime_He3[j] = cycleStart_he3;
	  cyclevalveopen_He3[j] = cyclevalveopenhe3;
	  cyclevalveclose_He3[j] = cyclevalveclosehe3;
	}
	...
	for(ULong64_t j=0; j < He3Event ; j++) {
	  ...
	  if (tIsUCN_he3 >0 && tUnixTime_he3 >20e6){
	    UCN_rate_He3-> Fill (tUnixTime_he3);
	  }
	  if(tIsUCN_he3 >0 && cycleStartTimes_He3.size() > 0){
	    // check which cycle this is in...
	    for(int i = 0; i < cycleStartTimes_He3.size()-1; i++){
	      if(tUnixTime_he3 >= cycleStartTimes_He3[i] && tUnixTime_he3 <= cycleStartTimes_He3[i+1]){
		numberEventsPerCyclehe3[i]++;
		//	break;
	      }
	      if (tUnixTime_he3 >= cyclevalveopen_He3[i] && tUnixTime_he3 < cyclevalveclose_He3[i]){
	...
	if(cycleStartTimes_He3.size() > 0){
	  for( int i = 0; i < cycleStartTimes_He3.size()-1; i++){
	    //  cout << cycleStartTimehe3[i] << " " << numberEventsPerCyclehe3[i] << endl;
	    UCN_per_cycle_He3_Whole_Sequence->Fill(cycleStartTime_He3[i],numberEventsPerCyclehe3[i]);
	    UCN_per_cycle_He3_valveOpen->Fill(cyclevalveopen_He3[i],numberEventsPerCyclehe3valveOpen[i]);
	  }
	...
	*/
	hfile.cd();
      ...
      std::ostringstream runnamehe3;
      runnamehe3 << "UCN_rate_He3_run" << midasrun;
	...
	He3CountsCycleStart = numberEventsPerCyclehe3[i];
	He3CountsValveOpen = numberEventsPerCyclehe3valveOpen[i];

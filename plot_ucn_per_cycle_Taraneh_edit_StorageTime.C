...
//             UCN 2017 analysis
//********************************************************
// IN THIS VERSION I LOOP OVER ALL THE FILES WITH THE FITS
// I ALSO INCLUDED THE RATE HISTOGRAM FOR EACH RUN.
// THE ERRORS ASSOCIATED WITH THE FIT PARAMETERS ARE ALSO
// INCLUDED.
//*******************************************************
...
void plot_ucn_per_cycle_Taraneh_edit_StorageTime(){  
  // Get the file
  char *filename[200];
  ...
  TFile hfile ("outputTree_StorageTime.root", "RECREATE");
  ...
  std::vector <double> Li6CountsValveOpen;
  for(int i = 0; i < StartofCycle.size(); i++) Li6CountsValveOpen.push_back(0.);
  std::vector <double> Li6CountsCycleStart;
 for(int i = 0; i < StartofCycle.size(); i++) Li6CountsCycleStart.push_back(0.);
 ...
 TH2F *psd_vs_ql_withcuts[16];
  ...
  for(int i = 0; i < StartofCycle.size(); i++) Baseline.push_back(0.);
  ...
  std::vector <double> BaselineErr;
  for(int i = 0; i < StartofCycle.size(); i++) BaselineErr.push_back(0.);
  std::vector <double> BaselineDuringIrradErr;
  for(int i = 0; i < StartofCycle.size(); i++) BaselineDuringIrrad.push_back(0.);
  std::vector <double> AmplitudeErr;
  for(int i = 0; i < StartofCycle.size(); i++) AmplitudeErr.push_back(0.);
  std::vector <double> RiseTimeErr;
  for(int i = 0; i < StartofCycle.size(); i++) RiseTimeErr.push_back(0.);
  std::vector <double> FallTimeErr;
  for(int i = 0; i < StartofCycle.size(); i++) FallTimeErr.push_back(0.);
  std::vector <double> DELAYErr;
  for(int i = 0; i < StartofCycle.size(); i++) DELAYErr.push_back(0.);
  ...
  // *************************************************************
  Int_t StorageTimeFiles[173] = {532, 533, 534 ,535, 536, 537, 538, 540, 541, 562, 563, 564, 565, 566, 567, 568, 569, 570, 571, 572, 573, 574, 575, 576, 577, 578, 579, 580, 581, 597, 598, 599, 600, 601, 602, 605, 606, 607, 608, 609, 610, 611, 612, 613, 662, 663, 664, 665, 666, 667, 668, 669, 670, 671, 672, 673, 675, 676, 677, 678, 679, 680, 681, 682, 683, 684, 685, 686, 687, 688, 689, 690, 692, 693, 695, 696, 697, 698, 699, 700, 701, 702, 703, 704, 705, 706, 707, 708, 709, 710, 711, 712, 713, 714 , 715, 716, 717, 718,719, 720, 721, 722, 723, 724, 725, 726, 727, 728, 729, 730, 731, 732, 733, 734, 735, 736, 737, 738, 739, 740, 741, 742, 743, 744, 745 , 746, 747, 748, 749, 750,751, 752, 753, 754, 755, 756, 757, 758, 759, 760, 761, 762, 763, 764, 777, 780, 781, 782, 783, 784, 785, 786, 791, 803, 813, 819, 821, 827, 832, 834};
  Int_t StorageTimeFileCounter =0; 
  Int_t total_counter = 0 ;
  Int_t fit_counter = 0;
  Inst_t total_cycle = 0;
  Int_t failfit_cycle = 0;
  for ( midasrun =524; midasrun <838; midasrun++){

    // I WANT TO HAVE ONLY THE STORAGE LIFETIME MEASUREMENTS
	...
	Li6CountsCycleStart.push_back(numberEventsPerCycle[i]);
	Li6CountsValveOpen.push_back(numberEventsPerCycleValveOpen[i]);
	...
	BaselineDuringIrradErr.push_back(baseline_during_irrad_Err[i]*BinWidth*prdcur_ave[i]);
	BaselineErr.push_back(baselineErr[i]*BinWidth);
	AmplitudeErr.push_back(ampErr[i]);
	RiseTimeErr.push_back(tauRiseErr[i]);
	FallTimeErr.push_back(tauFallErr[i]);
	DELAYErr.push_back(delayErr[i]);
      }

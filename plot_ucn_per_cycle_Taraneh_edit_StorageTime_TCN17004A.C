...
void plot_ucn_per_cycle_Taraneh_edit_StorageTime_TCN17004A(){  
  // Get the file
  char *filename[200];
  ...
  //TFile hfile ("outputTree_StorageTime_17004A.root", "RECREATE");
  TFile hfile ("blah.root", "RECREATE");
 ...
 TH2F *psd_vs_ql_withcuts[16];
  ...
  Int_t StorageTimeFiles[173] = {562, 563, 564, 565, 566, 567,568, 569, 570, 571, 572};
  ...
  for ( midasrun =524; midasrun <838; midasrun++){
	...
	cout << "CYCLE START " << cycleStart << "VALVE OPEN " << valveOpenTime << endl;
	numcycle=j+1;
      }
	...
	//cout << "delay " << cycleDelayTimes[i] << endl;
	AVE_B1CUR = b1_ave[i];

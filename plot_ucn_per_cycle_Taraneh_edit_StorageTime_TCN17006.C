...
void plot_ucn_per_cycle_Taraneh_edit_StorageTime_TCN17006(){  
  // Get the file
  char *filename[200];
  ...
  TFile hfile ("outputTree_StorageTime_17006.root", "RECREATE");
 ...
 TH2F *psd_vs_ql_withcuts[16];
  ...
  Int_t StorageTimeFiles[173] = {595, 596, 597, 598, 599,600, 601, 602};
  //Int_t StorageTimeFiles[173] = {595, 596};
	...
	for(ULong64_t j=0; j < beamlineEvent ;j++) {
	  ...
	  if (timestamp_beamline > irradiationStartTime[i] && timestamp_beamline < cycleStartTimes[i]) {
	    ...
	    if (B1V_KSM_PREDCUR >0 ){
	      ...
	      cout << "inside " << i << " " <<prdcur_ave[i] << endl;
	      counter_cur++;
	    }

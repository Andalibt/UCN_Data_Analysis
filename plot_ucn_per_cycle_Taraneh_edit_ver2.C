...
void plot_ucn_per_cycle_Taraneh_edit_ver2(std::string infile){
  ...
  double prdcur_ave=0, tnim_ave=0, b1_ave=0, targettemp1_ave = 0, targettemp2_ave=0;  
 ...
 cout << "  MAXIMUM B1 CURRENT DURING IRRADIATION:" << endl;
  ...
  // max and min is the same for B1 current. Does it make any sense?
 ...
 cout<<"**********************************************************"<<endl; 
  ...
  for(ULong64_t j=0;j<eventTot;j++) {
	...
	//avets12_temperature[i]=avets12_temperature[i]/counter;
	//	cout << i <<" " << counter << " " << oldi <<endl;
	avets12_temperature_valveOpen[i]+=UCN_ISO_TS12_RDTEMP;
	counter_2++;
	oldi_2=i;
	//	cout << avets12_temperature[i]<< " " << UCN_ISO_TS12_RDTEMP << endl;

...
//  Li6_Fit = ( x[0] > par[0] && x[0] < par[2] )*par[1]*par[4] + (x[0] > par[2] && x[0] < par[11])*( par[3] + ( par[6] /**( 2./par[9] )*/ * exp((par[7] + par[8]*par[8]/par[9]/2. - x[0]+par[10] )/( par[9])) * TMath::Erfc ((par[7] + par[8]*par[8]/par[9] - x[0]+par[10] )/TMath::Sqrt(2.)/par[8])))+ (x > par[11])*par[12] ;
  ...
  Double_t t = par[2] + par[8] + par[10];
  Li6_Fit = par[3] + (x[0] > par[0] && x[0] < par[2]) * par[1]*par[4] + (x[0] > t) * par[6]*(1 - exp(-(x[0] - t)/par[7]))*exp(-(x[0] - t)/par[9]);
...
void plot_ucn_per_cycle_Wolfgang_edit_ver7(){  
  // Get the file
  char *filename[200];
  ...
  TFile hfile ("outputTree_Wolfgang.root", "RECREATE");
  ...
  for ( midasrun =532 ; midasrun <540; midasrun++){
	...
	Li6_Fit_Func -> SetParName(8, "Valve open delay");
	Li6_Fit_Func -> SetParName(9, "Fall time");
	Li6_Fit_Func -> SetParName(10, "delay");
	Li6_Fit_Func -> SetParName(11, "cycle valve close time");
	Li6_Fit_Func -> SetParName(12, "Unused");
	//Li6_Fit_Func -> SetParameter(0, 0);
	Li6_Fit_Func -> FixParameter(0, irradiationStartTime[i]);
	Li6_Fit_Func -> SetParameter(1, 1);
	Li6_Fit_Func -> SetParLimits(1, 0, 10);
	...
	Li6_Fit_Func -> SetParLimits(6, 0, 10000);
	Li6_Fit_Func -> SetParameter(7, 2);
	Li6_Fit_Func -> SetParLimits(7, 1e-3, 10);
	Li6_Fit_Func -> FixParameter(8, delayTimeArray[i]);
	Li6_Fit_Func -> SetParameter(9, 20);
	Li6_Fit_Func -> SetParLimits(9, 1e-3, 100);
	Li6_Fit_Func -> SetParameter(10, 3);
	Li6_Fit_Func -> SetParLimits(10, 0, 10);
	Li6_Fit_Func -> FixParameter(11, cyclevalveclose[i]);
	Li6_Fit_Func -> FixParameter(12, 4);

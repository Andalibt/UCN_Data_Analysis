...
void StorageTime_17009A_Taraneh_edit_ver1(){
  gStyle->SetOptFit(1111);
  // READ THE TREE
  
  TFile *fin = new TFile("outputTree_StorageTime_17009A.root","READ");
  if ( fin->IsOpen() ){
  ...
  TH2* StorageTime = new TH2F("StorageTime", "Counts vs Delay Time", 1000, 0., 180. , 1000,  0., 600000);
  
  // HISTOGRAM TO SHOW THE TOTAL UCN (ONLY) COUNTS VS THE ISOPURE TEMPERATURE
  TH2* StorageTemp = new TH2F("StorageTemp", "Counts vs Temperture", 1000, 0.83, 1.3 , 1000,  0., 600000);
  
  // HISTOGRA TO SHOW THE TOTAL UCN (ONLY) COUNTS VS THE RUN NUMBER
  TH2* UCN = new TH2F("UCN", "UCN vs run#", 1000, 690, 770 , 1000,  0., 600000);
  
  // HISTOGRAM TO SHOW THE AVERAGE ISOPURE TEMPERATURE VS THE RUN NUMBER
  TH2* runTemp = new TH2F("UCN", "Temperature vs run#", 1000, 690, 770,1000, 0.83, 1.1);
  
  // HISTOGRAM TO SHOW THE TOTAL IRRADIATION TIME VS THE AVERAGE ISOPURE TEMPERATURE
  TH2* IrradTemp = new TH2F("UCN", "Irradiation Time vs Temperature", 1000, 0, 200 , 1000, 0.83, 1.3);
  
  // HISTOGRAM TO SHOW THE AVERAGE ISOPURE TEMPERATURE VS BEAM CURRENT
  TH2* TempBeam = new TH2F("UCN", "Temperature vs Beam Current", 1000, 0.83, 1.3 , 1000, 99, 160);
  
  // HISTOGRAM TO SHOW THE THE TOTAL UCN (ONLY) COUNTS VS AVERAGE BEAM CURRENT
  TH2* UCNBeam = new TH2F("UCN", "UCN Counts vs Beam Current" , 1000, 99, 160 , 1000 , 0 , 600000 );
  
  // HISTOGRAM TO SHOW THE TOTAL UCN (ONLY) COUNT VS IRRADIATION TIME
  TH2* UCNIrrad = new TH2F("UCN", "UCN Counts vs Irradiation Time" , 1000, 0, 200 , 1000 , 0 , 600000 );
  ...
  text -> SetTextAngle(0);
  text -> DrawText(0.4, 0.8, "Exp #: TCN17009A");
  ...
  TGraphErrors *gr = new TGraphErrors(68, delayTimeArray, UCNIntegralArray, 0 , UCNIntegralErrArray);

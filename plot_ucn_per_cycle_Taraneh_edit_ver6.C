...
Double_t Li6FitFunc (Double_t *x, Double_t *par){
  ...
  Li6_Fit = ( x[0] > par[0] && x[0] < par[2] )*par[1]*par[4] + (x[0] > (par[2]))*( par[3] + ( par[6] /**( 2./par[9] )*/ * exp((par[7] + par[8]*par[8]/par[9]/2. - x[0]+par[10] )/( par[9])) * TMath::Erfc ((par[7] + par[8]*par[8]/par[9] - x[0]+par[10] )/TMath::Sqrt(2.)/par[8])));
...
void plot_ucn_per_cycle_Taraneh_edit_ver6(){  
  // Get the file
  char *filename[200];
  ...
  TFile hfile ("outputTree_ver6.root", "RECREATE");
  ...
  Double_t Nbins_li6 = ( maxXrange - minXrange )/1000; 
  ...
  TH1* UCN_rate_li6= new TH1F("UCN_rate_Li6", "UCN-rate Histogram",1000 , minXrange, maxXrange);
  ...
  TH1* UCN_rate_He3= new TH1F("UCN_rate_He3", "UCN-rate Histogram", 1000 , minXrange, maxXrange);
  ...
  for ( midasrun =525 ; midasrun <528 ; midasrun++){
	...
	if (timestamp_beamline > maxXrange)
      ...
      UCN_rate_li6 -> Reset();
      UCN_rate_li6 -> SetBins(1000, minXrange , maxXrange);
	...
	TF1 *Li6_Fit_Func = new TF1("Li6FitFunc", Li6FitFunc ,min_range[i] , max_range[i] , 11);
	//	// par[0] : Background
	// par[0] : irradiation start time
	...
	Li6_Fit_Func -> SetParameter(6, Li6Max[i]/2);
	Li6_Fit_Func -> FixParameter(7, mu_li6[i]);
	Li6_Fit_Func -> SetParameter(8, 0.1);
	Li6_Fit_Func -> SetParameter(9, 39.8);
	Li6_Fit_Func -> SetParameter(10, 10.6);
	...
	//Li6_Fit_Func-> Draw("");
	//TFitResultPtr fitr = UCN_rate_li6->Fit(Li6_Fit_Func, "S", "" , minXrange, maxXrange);
	//int status = int ( fitr );
	...
	BaselineDuringIrrad.push_back(baseline_during_irrad[i]);
	BaselineAfterIrrad.push_back(baseline_after_irrad[i]);

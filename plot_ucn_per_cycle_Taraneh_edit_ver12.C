...
void plot_ucn_per_cycle_Taraneh_edit_ver12(){  
  // Get the file
  char *filename[200];
  ...
  TFile hfile ("outputTree_ver12.root", "RECREATE");
 ...
 TH2F *psd_vs_ql_withcuts[16];
  ...
  for ( midasrun =524; midasrun <838; midasrun++){
	...
	if (tPSD_li6 > PSDThreshold && tChargeL_li6 > QLongThreshold){
      ...
      for ( int i = 0 ; i < cycleStartTimes.size(); i++ ){
	irradStartTimes[i] = irradiationStartTime[i];
	StartofCycle[i] = cycleStartTimes[i];
	cycleValveOpenTimes[i] = cyclevalveopen[i];
	cycleValveCloseTimes[i] = cyclevalveclose[i];
	AVE_B1CUR[i] = b1_ave[i];
	AVE_PRDCUR[i] = prdcur_ave[i];
	AVE_TNIM[i] = tnim_ave[i];
	AVE_TARGETTEMP1[i] = targettemp1_ave[i];
	AVE_TARGETTEMP2[i] = targettemp2_ave[i];	   
	AVE_WTEMP1[i] = wtemp1_ave[i];
	AVE_WTEMP2[i] = wtemp2_ave[i];
	AVE_COL2UP[i] = col2up_ave[i];
	AVE_COL2DOWN[i] = col2down_ave[i];
	AVE_COL2RIGHT[i] = col2right_ave[i];
	AVE_COL2LEFT[i] = col2left_ave[i];
	AVE_TS12_IRRAD[i] = avets12_temperature_irrad[i];
	AVE_TS12_VALVEOPEN[i] = avets12_temperature_valveOpen[i];
	AVE_FM1[i] = fm1_ave[i];
	AVE_FM4[i] = fm4_ave[i];
	AVE_FM5[i] = fm5_ave[i];
	MAX_B1CUR[i] =  b1_max[i] ;
	MIN_B1CUR[i] =  b1_min[i] ;
	MAX_PRDCUR[i] =  prdcur_max[i] ;
	MIN_PRDCUR[i] =  prdcur_min[i] ;
	MAX_TNIM[i] =  tnim_max[i] ;
	MIN_TNIM[i] =  tnim_min[i] ;
	MAX_TARGETTEMP1[i] =  targettemp1_max[i] ;
	MIN_TARGETTEMP1[i] =  targettemp1_min[i] ;
	MAX_TARGETTEMP2[i] =  targettemp2_max[i] ;
	MIN_TARGETTEMP2[i] =  targettemp2_min[i] ;
	MAX_WTEMP1[i] =  wtemp1_max[i] ;
	MIN_WTEMP1[i] =  wtemp1_min[i] ;
	MAX_WTEMP2[i] =  wtemp2_max[i] ;
	MIN_WTEMP2[i] =  wtemp2_min[i] ;
	MAX_COL2UP[i] =  col2up_max[i] ;
	MIN_COL2UP[i] =  col2up_min[i] ;
	MAX_COL2DOWN[i] =  col2down_max[i] ;
	MIN_COL2DOWN[i] =  col2down_min[i] ;
	MAX_COL2RIGHT[i] = col2right_max[i] ;
	MIN_COL2RIGHT[i] = col2right_min[i] ;
	MAX_COL2LEFT[i] =  col2left_max[i] ;
	MIN_COL2LEFT[i] =  col2left_min[i] ;
	MAX_TS12_IRRAD[i] =  maxts12_temperature_irrad[i] ;
	MIN_TS12_IRRAD[i] =  mints12_temperature_irrad[i] ;
       	MAX_TS12_VALVEOPEN[i] =  maxts12_temperature_valveOpen[i];
       	MIN_TS12_VALVEOPEN[i] =  mints12_temperature_valveOpen[i];
	MAX_FM1[i] = fm1_max[i];
	MIN_FM1[i] = fm1_min[i];
	AVE_LND[i] = LND_ave[i];
	MAX_LND[i] = LND_max[i];
	MIN_LND[i] = LND_min[i];
	Li6CountsCycleStart[i] = numberEventsPerCycle[i];
	Li6CountsValveOpen[i] = numberEventsPerCycleValveOpen[i];
	BaselineDuringIrrad[i] = baseline_during_irrad[i]*BinWidth*prdcur_ave[i];
	Baseline[i] = baseline[i]*BinWidth;
	Amplitude[i] = amp[i];
	RiseTime[i] = tauRise[i];
	FallTime[i] = tauFall[i];
	DELAY[i] = delay[i];
	BaselineDuringIrradErr[i] = baseline_during_irrad_Err[i]*BinWidth*prdcur_ave[i];
	BaselineErr[i] = baselineErr[i]*BinWidth;
	AmplitudeErr[i] = ampErr[i];
	RiseTimeErr[i] = tauRiseErr[i];
	FallTimeErr[i] = tauFallErr[i];
	DELAYErr[i] = delayErr[i];
	outputTree -> Fill();
      }
